    ...
    <section id="certifications" className="section-y bg-steel-100/40">
      <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-12 flex max-w-4xl flex-wrap items-center justify-center gap-4 sm:gap-6">
          {CODES_AND_STANDARDS.map((cert, i) => (
            // Static badge: not a link, so no hover lift.
            <Reveal
              key={cert}
              delay={(i % 4) * 70}
              className="flex h-20 min-w-[8rem] items-center justify-center rounded-lg border-2 border-navy-900/10 bg-white px-4 text-center text-lg font-bold tracking-wide text-navy-900 shadow-sm"
            >
              {cert}
            </Reveal>
          ))}
